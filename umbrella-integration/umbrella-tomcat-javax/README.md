...
For `jakarta.servlet` applications (Tomcat 10.1 or newer) use `umbrella-tomcat`
instead. The `SameSite` cookie attribute is not supported by `javax.servlet` and
is ignored if the server sets one.
